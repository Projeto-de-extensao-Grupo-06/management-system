...
export default function useClients(): UseClientsReturn {
  ...
  const createClient = useCallback(
    async (data: ClientSchemaType): Promise<void> => {
      try {
        ...
        if (axiosError.response?.status === 409) {
          ...
          if (duplicateField) {
            ...
            errorMsg =
              message ||
              "Este cliente já existe no sistema. Verifique os dados.";
          }
        } else {
          errorMsg =
            axiosError.response?.data?.message ||
            "Erro ao criar cliente. Verifique os dados.";
  ...
  const updateClient = useCallback(
    async (id: number, data: ClientSchemaType): Promise<Client> => {
      try {
